...
async fn handle_key(app: &mut App, key: KeyEvent) -> Result<bool> {
    ...
    match key.code {
        ...
        KeyCode::Down => {
            if app.tab == Tab::Chain && app.chain_cursor + 1 < app.chain_rows.len() {
                app.chain_cursor += 1;
            }
        }
        KeyCode::Up => {
            if app.tab == Tab::Chain && app.chain_cursor > 0 {
                app.chain_cursor -= 1;
            }
        }
        _ => {
...
fn ui<B: Backend>(f: &mut Frame, app: &mut App) {
    ...
    // Main area
    match app.tab {
        Tab::Dashboard => render_dashboard::<B>(f, chunks[1], app),
        Tab::Chain => render_chain::<B>(f, chunks[1], app),
        Tab::Mempool => render_mempool::<B>(f, chunks[1], app),
        Tab::Mine => render_mine::<B>(f, chunks[1], app),
        Tab::HashDemo => render_hashdemo::<B>(f, chunks[1], app),
    }
...
fn render_dashboard<B: Backend>(f: &mut Frame, area: Rect, app: &App) {
    let chunks = Layout::default()
...
fn render_chain<B: Backend>(f: &mut Frame, area: Rect, app: &App) {
...
fn render_mempool<B: Backend>(f: &mut Frame, area: Rect, app: &App) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(5),
...
fn render_hashdemo<B: Backend>(f: &mut Frame, area: Rect, app: &App) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(8),
